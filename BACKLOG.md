...
- [x] Verify `gotk bench` numbers are within 5% of Linux/macOS on the same corpus (branch `sprint-14/windows-bench-parity`, PR #54, 2026-05-11). First CI parity run reported +0.00pp drift across all 21 fixtures — filters produce byte-identical output between Linux and Windows, no CRLF / path-separator / encoding leaks into the cleaned stream.
...
### Build — Follow-ups from the filter review

- [x] **Pre-existing gap found while testing #71**: `strictErrorPrefix` only matched a bare `error:`, so labelled compiler diagnostics inside source-grep content (tsc `error TS2304:`, rustc `error[E0433]:`) were demoted to Info — a real error that could then be dropped. The keyword now accepts an optional diagnostic code, in brackets or as a trailing token. Same treatment for `strictWarningPrefix`.
- [x] **Escape hatch for the Jest drop-on-pass.** Dropping application logs is the most destructive behaviour in the codebase, so it gets a switch: `[filters] jest_drop_console_on_pass`, default `true`. Command-specific filters had no config access, so `detect` gained an `Options` struct and `FiltersForWithOptions`, mirroring the existing `BuildChain` / `BuildChainWithExit` and `Summarize` / `SummarizeWithContext` pairs. `FiltersFor` keeps its signature and delegates with `DefaultOptions()`, so the 20-odd existing call sites are untouched. Verified end to end: default -96%, disabled -74% with the logs kept.
